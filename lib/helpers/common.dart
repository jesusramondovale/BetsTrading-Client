...
class Common {
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(
                ...
                obscureText: true,
                onChanged: (value) {
                  isButtonEnabled.value = value.trim().length >= 12;
                },
                style: GoogleFonts.montserrat(color: Colors.white),
                decoration: InputDecoration(
                  labelText: LocalizedStrings.of(context)!.get('password') ?? "Password",
                    ...
                    ElevatedButton(
                      ...
                      child: FittedBox(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text(
                              intl.NumberFormat.compact().format(coinsAmount).toString(),
                              style: GoogleFonts.montserrat(
                                fontSize: 25,
                            ...
                            const Icon(Icons.arrow_forward_ios_rounded, size: 28, color: Colors.white),
                            const SizedBox(width: 8),

                            Text(
                              intl.NumberFormat.compact().format(currencyAmount).toString(),
                              style: GoogleFonts.montserrat(
                                fontSize: 25,
                                fontWeight: FontWeight.w300,
                                color: Colors.white,
  ...
  Future<bool?> popConfirmOperationDialog(
      BuildContext aContext, double betAmount, String betIcon) async {
        ...
        return PopScope(
          ...
          child: AlertDialog(
            ...
            shape:
                RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
            title: Text(
              LocalizedStrings.of(context)!.get('confirmOperation') ??
                  "Confirm operation",
              textAlign: TextAlign.center,
              maxLines: 1,
              style: GoogleFonts.montserrat(
            ...
            content: Text(
              LocalizedStrings.of(context)!.get('confirmBet') ??
                  "Slide to confirm the operation",
              textAlign: TextAlign.center,
              style: GoogleFonts.montserrat(
                fontSize: 18,
                color: Colors.white70,
  ...
  double calculateMaxFontSize(
      String text, FontWeight fontWeight, double maxWidth) {
    double fontSize = 18;
  ...
  Bet generateRandomBet([double? odds]) {
    ...
    final originValue =
        random.nextDouble() * 1000; // Valor original aleatorio entre 0 y 1000
    final currentValue =
        random.nextDouble() * 1000; // Valor actual aleatorio entre 0 y 1000
    final targetValue = originValue +
        (random.nextDouble() * 100); // Valor objetivo mayor que el original
    final targetMargin =
        random.nextDouble() * 20; // Margen de objetivo aleatorio entre 0 y 20%
  ...
  List<RectangleZone> getRectangleZonesFromBetZones(
      List<BetZone> betZones, double current) {
  ...
  List<Candle> generateSinusoidalCandles(
    ...
    for (int i = 0; i < count; i++) {
      ...
      double close = centerValue +
          sinValue +
          (Random().nextBool() ? 1 : -1) * Random().nextDouble() * 5;
  ...
  bool isPasswordValid(String password) {
    final lengthOK = password.length >= 12;
  ...
  void showFloatingSnack(BuildContext context, String text, {Color backgroundColor = Colors.green , int theDuration = 4, bool showIcon = false}) {
    ...
    Future.delayed(Duration(milliseconds: theDuration*1000 + 400), () {
      overlayEntry.remove();
    });
  }
...
class _FloatingSnackState extends State<_FloatingSnack> with SingleTickerProviderStateMixin {
